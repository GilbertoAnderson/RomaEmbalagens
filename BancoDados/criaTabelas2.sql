...
begin		
	...
	/*
		...
		insert into tblProdutoItem( idProduto, idItem, idStatus, Nome, Quantidade, Observacao,  valor)
		values( (select idProduto from tblProduto where Nome = 'Caixa Pizza 25 Quadrada' ),
                ...
                'Caixa Pizza 25 Quadrada', 500, 'observacao', 10)
...
begin		
	 ...
	 * 
		...
		select * from  tblOrcamentoItem
	     ...
	     insert into tblOrcamentoItem( idOrcamento, idProduto,nome, Sequencial, Quantidade, ValorUnitario, Observacao)
	                 values(
				             ...
				             (select idProduto   from tblProduto    where Nome   = 'Caixa Pizza 25 Quadrada'),
	                           'Caixa Pizza 25 Quadrada', 1, 100, 3.00, 'observacao')

	     insert into tblOrcamentoItem( idOrcamento, idProduto,nome, Sequencial, Quantidade, ValorUnitario, Observacao)
	                 values(
							 (select idOrcamento from tblOrcamento  where nrOrcamento   =  '250206001'),
				             (select idProduto   from tblProduto    where Nome   = 'Caixa Pizza Redonda'),
	                           'Caixa Pizza Redonda', 2, 50, 3.50, 'yyyyy')
